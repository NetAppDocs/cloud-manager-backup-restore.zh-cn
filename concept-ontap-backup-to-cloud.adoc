...
| Google Cloud 存储 | Google 内部 ONTAP 系统中的 Cloud Volumes ONTAP | Google 内部 ONTAP 系统中的 Cloud Volumes ONTAP 
...
* 如果您需要从已移至归档存储的备份文件还原卷数据(在使用ONTAP 9.10.1+时、AWS支持此功能)、则云提供商会额外收取每GiB检索费用和每请求费用。
...
Cloud Backup提供三种许可选项：按需购买(PAYGO)订阅、AWS Marketplace提供的年度合同以及自带许可证(BYOL)。如果您订阅了PAYGO、则可以免费试用30天。
