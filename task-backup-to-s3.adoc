...
如果您使用 3.9.15 或更高版本部署了 Connector ，则这些权限应已属于 IAM 角色。否则，您需要添加缺少的权限。

需要 AWS 还原权限:: IAM 角色需要以下 EC2 权限，以便为 Cloud Manager 提供启动，停止和终止 Cloud Restore 实例的权限：
+
--
[source, json]
----
...
----
--
AWS 部署所需的出站 Internet 访问:: Cloud Restore 实例需要出站 Internet 访问。如果您的虚拟或物理网络使用代理服务器访问 Internet ，请确保此实例可以通过出站 Internet 访问与以下端点联系。
+
--
[cols="43,57"]
|===
