...
+
如果您的集群运行的是 ONTAP 9.10.1 或更高版本，并且您使用 AWS 或 Azure 作为云存储，则还可以选择在一定天数后启用或禁用将备份分层到归档存储。

+
link:reference-azure-backup-tiers.html["了解有关使用 Azure 归档存储的更多信息"]。link:reference-aws-backup-tiers.html["了解有关使用 AWS 归档存储的更多信息"]。
